...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        header("Location: booking.php?nama_pemesan=$nama_pemesan&jenis_kelamin=$jenis_kelamin&nomor_identitas=$nomor_identitas&tipe_kamar=$tipe_kamar&tanggal_pesan=$tanggal_pesan&durasi=$durasi&diskon=$diskon&total_bayar=$total_bayar");
        exit();
...
function updateTotal() {
    ...
    // Ambil nilai durasi menginap
    const durasi = parseInt(document.getElementById('durasi').value) || 0;
    
    // Periksa apakah sarapan termasuk
    const termasukSarapan = document.getElementById('termasuk_sarapan').checked;

    let total = harga * durasi; // Hitung total berdasarkan harga per malam dan durasi
    let diskon = 0; // Inisialisasi diskon

    // Tambahkan diskon jika durasi lebih dari 3 hari
    if (durasi > 3) {
        diskon = total * 0.10; // Diskon 10% dari total
        total -= diskon; // Kurangi total dengan diskon
    }

    // Tambahkan biaya sarapan jika dipilih
    const biayaSarapanPerHari = 80000; // Biaya sarapan per hari
    if (termasukSarapan) {
        total += biayaSarapanPerHari * durasi; // Tambahkan biaya sarapan ke total
    }

    // Tampilkan hasil total bayar dan diskon
    const totalBayarElement = document.getElementById('total_bayar');
    if (totalBayarElement) {
        totalBayarElement.value = 'Rp ' + total.toLocaleString();
    }
}
